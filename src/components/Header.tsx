...
import { Link as ScrollLink } from 'react-scroll'; // react-scrollをインポート

const Header = () => {
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  const [isWideScreen, setIsWideScreen] = useState(false);
  const [isAnimating, setIsAnimating] = useState(false); // アニメーション中かどうかのフラグ

  // Drawer（メニュー）の表示/非表示を切り替える関数
  const toggleDrawer = () => {
    if (!isDrawerOpen) setIsAnimating(true); // 開く場合はアニメーションフラグを立てる
    setIsDrawerOpen((prev) => !prev); // 開閉状態を切り替え
  };
  ...
  useEffect(() => {
    if (!isDrawerOpen && isAnimating) {
      const timeout = setTimeout(() => {
        setIsAnimating(false); // アニメーション完了後にフラグをオフ
      }, 500); // アニメーション時間（0.5秒）
  ...
  useEffect(()=> {
    const handleResize = ()=> {
      setIsWideScreen(window.innerWidth >= 768); // 768px以上をwide screenと判定
    }

    handleResize(); // 初回ロード時の画面幅をチェック
    ...
    <header className="header">
      <div className="header-container">
        <div className="header-logo">
          <div className="header-logo-main"><ScrollLink to="fv" smooth={true} duration={500} offset={-80} className="header-nav-Link">WebWiseWords</ScrollLink></div>
          <div className="header-logo-sub">- TK's Portfolio -</div>
        </div>
         {/* メニューボタン */}
        <button className={`header-menu-button ${isDrawerOpen ? 'is-checked' : ''}`} type="button" onClick={toggleDrawer}></button>
          ...
          <nav className="header-nav">
            <ul className="header-nav-list">
              <li className="header-nav-item">
                <ScrollLink to="service" smooth={true} duration={500} offset={-80} className="header-nav-Link button">
              ...
              <li className="header-nav-item">
                <ScrollLink to="works" smooth={true} duration={500} offset={-80} className="header-nav-Link button">
                  <span className="button-icon">
                  ...
                  <span className="button-text">Works -</span>
                </ScrollLink>
              </li>
              <li className="header-nav-item">
                <ScrollLink to="about" smooth={true} duration={500} offset={-80} className="header-nav-Link button">
                  <span className="button-icon">
              ...
              <li className="header-nav-item">
                <ScrollLink to="flow" smooth={true} duration={500} offset={-80} className="header-nav-Link button">
                  <span className="button-icon">
                  ...
                  <span className="button-text">Work flow -</span>
                </ScrollLink>
              </li>
              <li className="header-nav-item">
                <ScrollLink to="message" smooth={true} duration={500} offset={-80} className="header-nav-Link button">
                  <span className="button-icon">
              ...
              <li className="header-nav-item">
                <ScrollLink to="contact" smooth={true} duration={500} offset={-80} className="header-nav-Link button button-bg">
                  <span className="button-icon">
                    <svg width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path
                        ...
                        className="button-icon-path button-icon-path-contact"
                      />
                    </svg>
                  </span>
              ...
              <li className="header-nav-item">
                <Link to="https://twitter.com/mosu_man" target="_blank" className="header-nav-Link button button-border">
                  <span className="button-icon">
